...
pub fn run(config: &Config) {
    ...
    app.connect_activate(move |app| build_ui(app, auto_paste));
...
fn build_ui(app: &gtk4::Application, auto_paste: bool) {
    // -- Install CSS --
    ...
    list_box.connect_row_activated(move |_, row| {
        ...
        *is_pasting_activate.borrow_mut() = true;
        ...
        if auto_paste && is_text {
            ...
            let text_for_paste = content.text().unwrap_or_default().to_string();
            let (tx, rx) = std::sync::mpsc::channel::<()>();
            std::thread::spawn(move || {
                paste::simulate_paste(&text_for_paste);
                let _ = tx.send(());
            });
